...
class DynamicFusionGate(nn.Module):
    ...
    def extract_window_statistics(self, attn_weights, window_size):
        """Extract statistics from attention weights using sliding windows (vectorized)."""
        B, H, L, _ = attn_weights.shape
        num_windows = L // window_size

        if num_windows == 0:
            # Fallback: sequence too short, treat as single window
            ...
            stats = torch.stack([mean, std, max_val, min_val], dim=-1).unsqueeze(2)
            return stats

        # Vectorized computation: reshape to windows
        attn_windowed = attn_weights[:, :, :num_windows*window_size, :].reshape(
            B, H, num_windows, window_size, L
        )

        # Compute statistics in parallel across all windows
        mean = attn_windowed.mean(dim=-1).mean(dim=-2)  # [B, H, num_windows]
        std = attn_windowed.std(dim=-1).mean(dim=-2)
        max_val = attn_windowed.max(dim=-1)[0].mean(dim=-2)
        min_val = attn_windowed.min(dim=-1)[0].mean(dim=-2)

        stats = torch.stack([mean, std, max_val, min_val], dim=-1)  # [B, H, num_windows, 4]
        return stats
...
class MultiScaleDynamicFusionGate(nn.Module):
    ...
    def expand_alpha_to_sequence(self, alpha_windows, window_size, target_length):
        """
        Expand per-window alpha to per-frame alpha using smooth interpolation.
        This avoids discontinuities at window boundaries.
        """
        B, H, num_windows, _ = alpha_windows.shape

        # Smooth interpolation (no discontinuities)
        alpha_flat = alpha_windows.squeeze(-1)  # [B, H, num_windows]
        alpha_expanded = F.interpolate(
            alpha_flat.unsqueeze(1),  # [B, 1, H, num_windows]
            size=(H, target_length),
            mode='bilinear',
            align_corners=True  # Align window centers to sequence positions
        ).squeeze(1).unsqueeze(-1)  # [B, H, target_length, 1]
    ...
    def forward(self, attn_1, attn_2):
        ...
        # Determine which scales are feasible
        ...
        alpha_stacked = torch.stack(alpha_scales, dim=-1)
        weights_expanded = weights.view(1, 1, 1, 1, -1)
        
        alpha_final = (alpha_stacked * weights_expanded).sum(-1)
...
class AdaptiveDifferentialAttention(nn.Module):
    ...
    def forward(self, x, attn_mask=None, global_step=None, window_size=None, return_alpha=False):
        ...
        attn_1 = attn_weights[:, :, 0]
        attn_2 = attn_weights[:, :, 1]
...
class HarmonicAwareGroupedConv(nn.Module):
    def __init__(self, in_channels, out_channels, stride=(1,2), n_erb_groups=4,
                 freq_bins=801, sample_rate=16000):
        super().__init__()
    ...
    def forward(self, x):
        ...
        if H != self.freq_bins:
            # Use nearest neighbor to preserve ERB band alignment (not bilinear)
            masks_list = []
            for j in range(self.n_groups):
                mask_j = masks[j:j+1].permute(0, 2, 1, 3)
                mask_j = F.interpolate(mask_j, size=(H, 1), mode='nearest')
...
class Net(nn.Module):
    """
    Main network optimized for n_fft=1600, hop=100.
    
    This gives:
    - 801 frequency bins (fine harmonic resolution: 10 Hz)
    - 321 time frames for 2 sec (long temporal context)
    
    Perfect for:
    - HarmonicAwareGroupedConv (needs fine frequency resolution)
    - Multi-scale differential attention (needs long sequences)
    - Transformer bottleneck (needs rich context)
    """
    def __init__(self, sample_rate=16000, use_swiglu=True, use_multiscale=False):
        super().__init__()
        # ? CHANGED: 601 ? 801 for n_fft=1600
        Freq = 801
        ...
        self.m1 = HybridTransformerBottleneck(
            ...
            max_seq_length=2000,
            input_frequency_bins=25,
            original_frequency_bins=Freq,  # Now 801
            use_multiscale=use_multiscale,
        )
